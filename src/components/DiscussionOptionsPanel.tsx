...
function SearchConfigSection({ disabled, searchConfig, onSearchConfigChange }: SearchConfigSectionProps) {
  ...
  const hasAdvancedCustom =
    (searchConfig.provider && searchConfig.provider !== defaultProvider) ||
    searchConfig.searchType !== 'web' ||
    timing.eachRound ||
      ...
      {searchConfig.enabled && (
        <div className={`space-y-4 ${disabled ? 'opacity-50' : ''}`}>
          ...
          <div className="space-y-3 p-3 bg-gray-800/50 rounded-lg">
            <div className="text-xs font-medium text-gray-300 mb-2">検索件数設定</div>

            {/* キーワードあたりの取得件数 */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs text-gray-400">キーワードあたりの取得件数</label>
                <span className="text-xs text-green-400 font-medium">{searchConfig.maxResults}件</span>
              </div>
              <input
                type="range"
                min="3"
                ...
                disabled={disabled}
                title={`キーワードあたりの取得件数: ${searchConfig.maxResults}件`}
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs text-gray-400">生成キーワード数</label>
                <span className="text-xs text-green-400 font-medium">{searchConfig.maxKeywords || 1}個</span>
              </div>
              <input
                type="range"
                min="1"
                ...
                disabled={disabled}
                title={`生成キーワード数: ${searchConfig.maxKeywords || 1}個`}
                className={`w-full h-1.5 bg-gray-600 rounded-lg appearance-none accent-green-500 ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}
              />
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-600/50">
              <span className="text-xs text-gray-400">1回の検索あたり</span>
              <span className="text-sm font-medium text-green-400">
                最大{searchConfig.maxResults * (searchConfig.maxKeywords || 1)}件
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <button
              ...
              className={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-colors ${
                showAdvanced ? 'bg-gray-700/50' : 'bg-gray-800/30 hover:bg-gray-700/30'
              } ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
            >
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-gray-300">詳細設定</span>
                {hasAdvancedCustom && <span className="w-1.5 h-1.5 rounded-full bg-green-400" />}
              </div>
              <svg
            ...
            {/* 詳細設定の展開領域 */}
            {showAdvanced && (
              <div className="space-y-3 pl-3 border-l-2 border-gray-600/50">
                {/* 検索プロバイダー選択 */}
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {searchConfig.provider === 'duckduckgo' && (
                      <span className="text-xs text-gray-500">Web検索のみ</span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <label className={`flex items-center gap-2 ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                      <input
                        ...
                        className="w-3.5 h-3.5 rounded border-gray-500 bg-gray-600 text-green-500 focus:ring-green-500 focus:ring-offset-0 disabled:opacity-50"
                      />
                      <span className="text-xs text-gray-300">AIが要求した時</span>
                    </label>
                  </div>
                </div>
                    ...
                    {searchConfig.fetchFullContent && (
                      <>
                        <input
                          ...
                          disabled={disabled}
                          title={`全文取得件数: ${Math.min(searchConfig.fullContentMaxResults || 3, searchConfig.maxResults)}件`}
                    ...
                    {searchConfig.relevanceFilter?.enabled && (
                      <>
                        <input
                          ...
                          disabled={disabled}
                          title={`関連性フィルタ閾値: ${Math.round((searchConfig.relevanceFilter?.threshold ?? 0.5) * 100)}%`}
